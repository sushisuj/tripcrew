...
from tripcrew.schemas import Attraction, Flight, Hotel, Restaurant
...
def _restaurant(estimated_cost_usd=None, name="Cervejaria Ramiro"):
    return Restaurant(name=name, city="Lisbon", estimated_cost_usd=estimated_cost_usd)
...
def test_picks_the_cheapest_priced_flight_not_the_first_one():
    ...
    budget = estimate_budget.func(flights=flights, hotel=None, attractions=[], restaurants=[], nights=4)
...
def test_no_flights_at_all_is_not_the_same_as_unpriced_flights():
    ...
    budget = estimate_budget.func(flights=[], hotel=None, attractions=[], restaurants=[], nights=4)
    assert budget.flights_usd == 0
    assert "flights" not in budget.unpriced_categories


def test_hotel_total_is_price_per_night_times_nights():
    budget = estimate_budget.func(flights=[], hotel=_hotel(price_per_night_usd=203.90), attractions=[], restaurants=[], nights=4)
    assert budget.hotel_usd == 203.90 * 4


def test_hotel_present_with_no_price_is_flagged_unpriced():
    budget = estimate_budget.func(flights=[], hotel=_hotel(price_per_night_usd=None), attractions=[], restaurants=[], nights=4)
...
def test_attractions_with_no_cost_data_are_flagged_unpriced_not_treated_as_free():
    ...
    budget = estimate_budget.func(flights=[], hotel=None, attractions=attractions, restaurants=[], nights=4)
...
def test_restaurants_with_no_cost_data_are_flagged_unpriced_not_treated_as_free():
    # Same real-world case as attractions: Geoapify never returns cost data
    # for catering places either, so this is what every actual run hits.
    restaurants = [_restaurant(estimated_cost_usd=None), _restaurant(estimated_cost_usd=None, name="Time Out Market")]
    budget = estimate_budget.func(flights=[], hotel=None, attractions=[], restaurants=restaurants, nights=4)
    assert budget.restaurants_usd == 0
    assert "restaurants" in budget.unpriced_categories


def test_restaurants_sums_only_the_priced_ones():
    restaurants = [_restaurant(estimated_cost_usd=12.0), _restaurant(estimated_cost_usd=28.0, name="Time Out Market")]
    budget = estimate_budget.func(flights=[], hotel=None, attractions=[], restaurants=restaurants, nights=4)
    assert budget.restaurants_usd == 40.0


def test_no_restaurants_at_all_is_not_flagged_unpriced():
    budget = estimate_budget.func(flights=[], hotel=None, attractions=[], restaurants=[], nights=4)
    assert budget.restaurants_usd == 0
    assert "restaurants" not in budget.unpriced_categories


def test_total_is_computed_from_the_parts_not_stated_separately():
    flights = [_flight(price_usd=544.67)]
    budget = estimate_budget.func(
        flights=flights,
        hotel=_hotel(price_per_night_usd=100.0),
        attractions=[_attraction(estimated_cost_usd=20.0)],
        restaurants=[_restaurant(estimated_cost_usd=30.0)],
        nights=4,
    )
    assert budget.total_usd == 544.67 + 400.0 + 20.0 + 30.0
...
def test_accepts_plain_dicts_the_way_crewai_actually_hands_them_over():
    ...
    # Flight/Hotel/Attraction/Restaurant instances, despite the args_schema
    # describing the nested shape -- confirmed by calling this directly and
    # hitting AttributeError on a dict before the model_validate() coercion
    # was added. This is what actually exercises the crew end to end, not
    # the typed-object calls above.
    ...
    restaurants = [{"name": "Cervejaria Ramiro", "city": "Lisbon", "estimated_cost_usd": 30.0}]
    budget = estimate_budget.func(flights=flights, hotel=hotel, attractions=attractions, restaurants=restaurants, nights=4)
    assert budget.total_usd == 544.67 + 400.0 + 20.0 + 30.0
