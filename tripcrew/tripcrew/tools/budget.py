...
from tripcrew.schemas import Attraction, Budget, Flight, Hotel, Restaurant
...
def estimate_budget(
    ...
    """Compute the trip's budget from actual flight, hotel, attraction, and
    restaurant data.

    Call this with the flights, hotel, attractions, and restaurants already
    gathered by the earlier research, don't add up a total yourself. Sums
    whatever price data actually came back, and says so honestly when it
    didn't.
    ...
    arithmetic instead. estimated_cost_usd on Attraction and Restaurant is
    None every time right now, since Geoapify doesn't return cost data for
    either (see attractions.py, restaurants.py), which used to mean
    attractions_usd was always 0 and nothing ever said so.
    unpriced_categories exists so that's visible instead of silent, and now
    covers restaurants the same way.
    ...
    # to, plain dicts, not Flight/Hotel/Attraction/Restaurant instances, even
    # though the args_schema built from these type hints describes the
    # nested shape. Confirmed by calling estimate_budget.run() directly and
    # hitting AttributeError on flights[0].price_usd. Coerce explicitly
    # instead of assuming the type hints are enforced for us.
    ...
    restaurants = [r if isinstance(r, Restaurant) else Restaurant.model_validate(r) for r in restaurants]
    ...
    priced_restaurants = [r for r in restaurants if r.estimated_cost_usd is not None]
    restaurants_total = sum(r.estimated_cost_usd for r in priced_restaurants)
    ...
    if restaurants and not priced_restaurants:
        unpriced_categories.append("restaurants")

    budget = Budget(
        flights_usd=flights_total,
        hotel_usd=hotel_total,
        attractions_usd=attractions_total,
        restaurants_usd=restaurants_total,
